...
/**
 * Importación masiva de pasajeros titulares con IA (solo Super).
 * Mismo flujo que proveedores: archivo o texto; preview y commit.
 */
const PassengerImportModal = ({ isOpen, onClose, onImported }) => {
  ...
  const handleAnalyze = async () => {
    ...
    setAnalyzing(true);
    try {
      const res = await api.post('/api/clients/import/preview', fd, {
      ...
      const rows = res.data.data?.passengers || [];
      setPreviewRows(rows);
      setSelected(new Set(rows.map((_, i) => i)));
      if (rows.length === 0) {
        setError('La IA no detectó pasajeros titulares válidos. Revisá DNI/pasaporte y email o teléfono.');
      }
    } catch (e) {
  ...
  const handleCommit = async () => {
    ...
    setError('');
    try {
      const res = await api.post('/api/clients/import/commit', { passengers: toSend });
      if (!res.data.success) {
  ...
  const idLabel = (row) => row.dni || row.passportNumber || '—';
    ...
    >
      ...
      >
        <div className="flex items-center justify-between px-5 py-4 border-b border-white/10">
          <div>
            <h2 className="text-lg font-semibold text-dark-100">Importar pasajeros titulares (IA)</h2>
            <p className="text-xs text-dark-400 mt-1">
              Solo Super. Imagen, CSV, Excel o texto; se crean solo titulares con nombre, apellido, DNI o pasaporte, y
              email o teléfono. Opcional en backend:{' '}
              <span className="font-mono">PASSENGERS_IMPORT_PROMPT_SUFFIX</span>.
            </p>
          </div>
            ...
            <>
              <div>
                ...
                <textarea
                  ...
                  placeholder="Pegá lista, CSV, columnas de Excel, notas…"
                />
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="text-[11px] text-dark-500 mt-1">
                    Imagen, CSV, Excel o texto. PDF: usá captura de pantalla.
                  </p>
                </div>
                <div>
            ...
            <>
              <div className="flex flex-wrap items-center justify-between gap-2">
                <p className="text-sm text-dark-300">
                  {previewRows.length} pasajero(s) titular(es) listo(s). Desmarcá los que no quieras importar.
                </p>
                ...
                <table className="w-full text-sm">
                  <thead className="bg-dark-700 sticky top-0">
                    <tr>
                      ...
                      <th className="px-2 py-2 text-left text-dark-300">Apellido</th>
                      <th className="px-2 py-2 text-left text-dark-300">DNI / Pasaporte</th>
                  ...
                  <tbody>
                    {previewRows.map((row, i) => (
                      <tr key={i} className="border-t border-white/5">
                        ...
                        <td className="px-2 py-2 text-dark-100">{row.surname}</td>
                        <td className="px-2 py-2 text-dark-300 font-mono text-xs">{idLabel(row)}</td>
                        <td className="px-2 py-2 text-dark-300">{row.email || '—'}</td>
                        <td className="px-2 py-2 text-dark-300">{row.phone || '—'}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              ...
              <div className="flex flex-wrap gap-2">
                ...
                >
                  {committing ? 'Importando…' : `Importar ${selected.size} pasajero(s)`}
                </button>
...
export default PassengerImportModal;
